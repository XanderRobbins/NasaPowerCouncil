...
class RollingRidgeModel:
    ...
    - Optional PCA dimensionality reduction (after scaling)
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        if self.use_pca:
            # Clamp n_components so we never ask for more than we have
            n_comp = min(self.pca_n_components, X_train_scaled.shape[1], X_train_scaled.shape[0])
            self.pca = PCA(n_components=n_comp)
            X_train_scaled = self.pca.fit_transform(X_train_scaled)
            logger.info(
                f"PCA: {self.pca.n_components_} components, "
                f"explained variance ratio={self.pca.explained_variance_ratio_.round(3).tolist()} "
                f"(total={self.pca.explained_variance_ratio_.sum():.3f})"
            )
    ...
    def get_feature_importance(self) -> pd.DataFrame:
        ...
        When PCA is enabled the coefficients live in component space, so
        we label them as PC1..PCn instead of the original feature names.
        """
        if self.use_pca and self.pca is not None:
            names = [f'PC{i+1}' for i in range(self.pca.n_components_)]
