"""
Backtest visualization with matplotlib and dark theme.
"""
import pandas as pd
import numpy as np
import matplotlib
...
class MatplotlibVisualizer:
    """
    Create visualizations for backtest results using matplotlib.
    ...
    def plot_equity_curve(self, results: pd.DataFrame, ax: Optional[plt.Axes] = None, use_index: bool = False):
        """Plot portfolio equity curve."""
        if ax is None:
            fig, ax = plt.subplots(figsize=(12, 4))

        x = range(len(results)) if use_index else results['date']
        ax.plot(x, results['portfolio_value'],
    ...
    def plot_drawdown(self, results: pd.DataFrame, ax: Optional[plt.Axes] = None, use_index: bool = False):
        """Plot drawdown chart."""
        if ax is None:
        ...
        x = range(len(results)) if use_index else results['date']
        ax.fill_between(x, drawdown * 100, 0,
                       color=self.colors['drawdown'], alpha=0.6)
        ax.plot(x, drawdown * 100,
    ...
    def plot_signals(self, results: pd.DataFrame, commodities: list,
                    ax: Optional[plt.Axes] = None, use_index: bool = False):
    ...
    def plot_positions(self, results: pd.DataFrame, commodities: list,
        ...
        for commodity in commodities:
            ...
            if position_col in results.columns:
                ax.plot(x, results[position_col],
                       label=commodity.capitalize(), alpha=0.8, linewidth=1.5)
    ...
    def create_full_report(self, results: pd.DataFrame, commodities: list,
        ...
        if trade_months:
            results_to_plot = results[results['date'].dt.month.isin(trade_months)].copy()
            logger.info(f"Filtered to {len(results_to_plot)} trading days in months {trade_months}")
        ...
        # Use index instead of dates if filtering to trade months
        use_index = trade_months is not None

        # Plot 1: Equity curve (top, spans both columns)
        ax1 = fig.add_subplot(gs[0:2, :])
        self.plot_equity_curve(results_to_plot, ax=ax1, use_index=use_index)

        # Plot 2: Drawdown
        ax2 = fig.add_subplot(gs[2, :])
        self.plot_drawdown(results_to_plot, ax=ax2, use_index=use_index)

        # Plot 3: Returns distribution
        ax3 = fig.add_subplot(gs[3, 0])
        self.plot_returns_distribution(results_to_plot, ax=ax3)

        # Plot 4: Rolling Sharpe
        ax4 = fig.add_subplot(gs[3, 1])
        self.plot_rolling_sharpe(results_to_plot, ax=ax4, use_index=use_index)

        # Plot 5: Signals
        ax5 = fig.add_subplot(gs[4, :])
        self.plot_signals(results_to_plot, commodities, ax=ax5, use_index=use_index)

        # Plot 6: Positions
        ax6 = fig.add_subplot(gs[5, :])
        self.plot_positions(results_to_plot, commodities, ax=ax6, use_index=use_index)
...
def plot_backtest_results(results: pd.DataFrame,
    ...
    Args:
        ...
        trade_months: List of months to include in visualization
    """
    visualizer = MatplotlibVisualizer()
    fig = visualizer.create_full_report(results, commodities, save_path, trade_months)
